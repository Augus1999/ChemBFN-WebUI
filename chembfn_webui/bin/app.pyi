...
with gr.Blocks(title="ChemBFN WebUI", analytics_enabled=False) as app:
    with gr.Row():
        with gr.Column(scale=1):
            with gr.Row(scale=1):
                btn = gr.Button("RUN", variant="primary", scale=2)
                stop = ToolButton("\u23F9", variant="stop", scale=1)
    ...
    stop.click(fn=None, inputs=None, outputs=None, cancels=[gen])
    btn_refresh.click(
        fn=_refresh,
...
def main() -> None:
    ...
    app.launch(
        ...
        css=".custom_footer {text-align:center;bottom:0;} .stop_btn {width:10px}",
    )
